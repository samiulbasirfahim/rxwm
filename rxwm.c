...
struct Client {
  ...
  int isfixed, isfloating, isurgent, neverfocus, oldstate, isfullscreen,
      issticky, isterminal, noswallow, allowkill;
  pid_t pid;
  char scratchkey;
  Client *next;
  Client *snext;
    ...
    if (text[i] == '^' && !isCode) {
      ...
      while (text[++i] != '^') {
        if (text[i] == 'c') {
          ...
          /* drw_rect(drw, rx + x, ry + vertpadbar / 2, rw,MIN(rh, bh -
           * vertpadbar), 1, 0); */
...
void drawbar(Monitor *m) {
  ...
  if ((w = m->ww - tw - stw - x) > bh) {
    ...
    if (m->sel) {
      /* drw_text(drw, x - 2 * sp, vertpadbar / 2, w, bh - vertpadbar, lrpad /
       * 2, m->sel->name, 0); */
      drw_setscheme(drw, scheme[m == selmon ? SchemeSel : SchemeNorm]);
      drw_text(drw, x - 2 * sp, vertpadbar / 2,
               (full_title_width
                    ? w
                    : (TEXTW(m->sel->name) >= w ? w : TEXTW(m->sel->name))),
               bh - vertpadbar, lrpad / 2, m->sel->name, 0);
      if (m->sel->isfloating)
...
void focusTeam(const Arg *arg) {
  ...
  Monitor *current_mon = selmon;
  unsigned int current_tagset = selmon->tagset[selmon->seltags];
    ...
    if (!m->tagwin) {
      if (topbar) {
        m->tagwin = XCreateWindow(
            dpy, root, m->wx + sp, m->by + bh + vp, m->mw / scalepreview,
            m->mh / scalepreview, 0, DefaultDepth(dpy, screen), CopyFromParent,
            DefaultVisual(dpy, screen),
            CWOverrideRedirect | CWBackPixmap | CWEventMask, &wa);
      } else {
        m->tagwin = XCreateWindow(
            dpy, root, m->wx + sp, m->by - (m->mh / scalepreview) + vp,
            m->mw / scalepreview, m->mh / scalepreview, 0,
    ...
    if (i < m->nmaster) {
      ...
      if (n - m->nmaster == 1) {
        resize(c, m->wx + mx + m->gappx, m->wy + my + m->gappx,
               mw - (2 * c->bw) - m->gappx, h - (2 * c->bw) - m->gappx * 2, 0);
      } else if (n - m->nmaster > 1) {
        resize(c, m->wx + mx, m->wy + my + m->gappx, mw - (2 * c->bw),
               h - (2 * c->bw) - m->gappx * 2, 0);
      } else {
        resize(c, m->wx + mx + m->gappx, m->wy + my + m->gappx,
               mw - (2 * c->bw) - m->gappx * 2, h - (2 * c->bw) - m->gappx * 2,
               0);
      }
      my += HEIGHT(c) + m->gappx;
      mfacts -= c->cfact;
      ...
      if ((i - m->nmaster) % 2) { // Even clients
        ...
        if (n - m->nmaster == n) {
          resize(c, m->wx + m->gappx, m->wy + ety + m->gappx,
                 tw - (2 * c->bw) - m->gappx, h - (2 * c->bw) - m->gappx * 2,
                 0);
        } else {
          resize(c, m->wx + m->gappx, m->wy + ety + m->gappx,
                 tw - (2 * c->bw) - m->gappx * 2,
...
void bottomstack(struct Monitor *m) {
  ...
  if (n == 1) {
    ...
    resize(c, m->wx + m->gappx, m->wy + m->gappx,
           m->ww - 2 * c->bw - 2 * m->gappx, m->wh - 2 * c->bw - 2 * m->gappx,
           0);
    return;
  }
  ...
  for (i = 0, mx = tx = 0, c = nexttiled(m->clients); c;
    ...
    if (i < m->nmaster) {
      ...
      resize(c, m->wx + mx + m->gappx, m->wy + m->gappx,
             w - (2 * c->bw) - 2 * m->gappx, mh - 2 * c->bw - 2 * m->gappx, 0);
      if (mx + WIDTH(c) < m->mw)
        mx += WIDTH(c) + m->gappx;
      mfacts -= c->cfact;
    } else {
      ...
      if (m->nmaster == 0) {
        resize(c, m->wx + tx + m->gappx, m->wy + mh + m->gappx,
               w - (2 * c->bw) - 2 * m->gappx,
               m->wh - mh - 2 * (c->bw) - 2 * m->gappx, 0);
      } else {
        resize(c, m->wx + tx + m->gappx, m->wy + mh,
               w - (2 * c->bw) - 2 * m->gappx,
               m->wh - mh - 2 * (c->bw) - m->gappx, 0);
      }
      if (tx + WIDTH(c) < m->mw)
        tx += WIDTH(c) + m->gappx;
      sfacts -= c->cfact;
...
void deck(Monitor *m) {
  ...
  if (n > m->nmaster) {
    ...
    mw = m->ww - m->gappx;
  for (i = my = 0, my = m->gappx, c = nexttiled(m->clients); c;
       c = nexttiled(c->next), i++)
    if (i < m->nmaster) {
...
void grid(Monitor *m) {
  ...
  for (i = 0, c = nexttiled(m->clients); c; c = nexttiled(c->next)) {
    ...
    resize(c, cx, cy, cw - 2 * c->bw + aw - m->gappx,
           ch - 2 * c->bw + ah - m->gappx, False);
    i++;
  }
}
